...
class ValidationModel():
    ...
    def predict(self, img):
        img = self.preprocess_cityscapes_vid(img)
    ...
    def create_label_colormap(self, no_class=7, dataset=None):
    ...
    def label_to_color_image(self, label):
        ...
        colormap = self.create_label_colormap()  # By default no_class = 7
        return colormap[label]


    def vis_segmentation(self,image_path, prediction, file_name=''):
        # image
        image = cv2.imread(image_path)
        all_images = image

        # Prediction
        prediction = self.label_to_color_image(prediction)
